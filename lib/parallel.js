...
//
// v2.2.6 (May 9 2026) — single press release inflation guard:
// - Rule #5 reinforced: single press release ≠ stablecoin core. Need consistent
//   product surface (multiple sources, dedicated section on website, public API).
// - Catches dLocal/Coins.ph type cases where Parallel saw a fresh PR and
//   inflated all axes 1-3 to 9/10 based on one announcement.
//
...
function buildResearchQuery(company, domain) {
  return [
    ...
    `  5. Temporal accuracy. Score TODAY's business, not 2-year-old news or future plans. SINGLE PRESS RELEASE ≠ STABLECOIN CORE: a single recent announcement (even from this month) does NOT make stablecoin a primary line. Score axes 1-3 based on the COMPANY'S CONSISTENT PRODUCT SURFACE (homepage, docs, dedicated landing page, public API). One PR + no dedicated product page = stablecoin is "experimental/partial" → axes 1-3 max=7.`,
