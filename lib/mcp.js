...
function registerMcpTools(server) {
  ...
  server.tool(
    ...
    "Poll a Parallel research task for COMPACT output (~600 bytes per company). Returns {cat, role, hk, axes:{1..8,10:{s,l,u}}, strat, ready, vol, conf}. ~10x smaller than parallel_get_result. Use for batch processing 10-30 companies. Atlas Comment / rationale not included — Claude synthesises those from compact axes after batch.",
    { taskId: z.string().describe("Task ID from parallel_research_start") },
    async ({ taskId }) => {
      try {
