...
// conversation-store.appendUserTurn expects text only, so we inject the
// content-blocks form directly. This is the ONE place where agent.js touches
// store internals. If conversation-store later gains a proper
// appendUserTurnRaw({content, from}) method, switch to that.
...
function appendToolResultUserTurn(toolResultContent) {
  ...
  try {
    ...
    if (fs.existsSync(STORE_PATH)) {
      ...
      // store.js shape — if file doesn't exist yet, init it
      data = {
        version: 1,
    ...
    console.error(`[agent] Failed to persist tool_result user turn: ${err.message}`);
    throw err;
  }
}
