...
//   Parallel:  5  (research_start, get_result, score, insight_start, research_wait)
...
function registerMcpTools(server) {
  ...
  server.tool(
    ...
    "Start a Parallel.ai deep research task on a company for Plexo BD scoring. Returns taskId; poll with parallel_get_result. Use processor='base' for high-priority companies (clientScore>=7.5), 'lite' otherwise.",
    {
  ...
  server.tool(
    "parallel_get_result",
    "Poll a Parallel research task for completion. Returns {status, done, failed, output}. When done=true, output contains the structured 10-axis scoring data.",
  ...
  server.tool(
    "parallel_score",
    "Run Parallel scoring with auto-selected processor based on the prior client score. Companies with clientScore>=7.5 get 'base' (deep), others 'lite'. Returns taskId; poll with parallel_get_result.",
    {
      company: z.string(),
  ...
  server.tool(
    ...
    "Start Parallel research and BLOCK until complete (or timeout). Returns full output directly — no polling needed. Use for batch scoring where you want the result inline. Default timeout: 180s.",
    {
      company: z.string(),
      domain: z.string().optional(),
