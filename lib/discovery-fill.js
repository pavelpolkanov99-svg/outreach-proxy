/**
 * Discovery Card fill scoring — structural approach v4
 *
 * "Filled" = partner actually typed/checked something NEW.
 * "Empty"  = only our pre-fill template content.
 *
 * Table scoring: row[0] always = header (skip). row[1..]: cells[0] = our label,
 * cells[1..] = partner input. Filled if any of cells[1..] has content.
 * For corridor-style tables (no label col): any cell with content = filled.
 */
...
function isBoilerplate(text) {
  ...
  for (const p of FIELD_LABEL_PATTERNS) { if (p.test(t)) return true; }
  for (const p of BOILERPLATE_PATTERNS) { if (p.test(t)) return true; }
...
// ── Table scoring: row[0] = header (skip), row[1..] = data ───────────────────
function scoreTable(rows) {
  let filled = 0;
  let total = 0;
  // Skip row[0] — always the header
  for (let i = 1; i < rows.length; i++) {
    const cells = rows[i].cells || [];
    if (cells.length === 0) continue;

    // cells[0] = row label (our pre-fill, e.g. "SME / Business Payments")
    // cells[1..] = partner-editable columns
    const dataCells = cells.slice(1);

    if (dataCells.length === 0) {
      // Single-column table — cells[0] itself is the answer
    ...
    // Multi-column: count the row, filled if any data cell has content
...
function scoreBlocksFill(blocks) {
  ...
  function walk(blist) {
    for (const b of blist) {
      ...
      // table: score as a unit using row-index logic
      if (type === "table") {
      ...
      // table_row: skip when encountered outside table context (shouldn't happen)
      if (type === "table_row") continue;
