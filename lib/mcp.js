...
function registerMcpTools(server) {
  ...
  // NOTE: customMessages MUST be an explicit z.object schema (not z.any()).
  // MCP SDK + zod-to-JSON-schema serialization drops free-form `any` objects
  // somewhere in the transit chain when using Streamable HTTP transport,
  // even though z.any() validates fine in pure zod. Confirmed via a debug
  // run on Apr 25, 2026 (campaign 408212): customMessages={note,fu1,fu2}
  // was passed but reached the proxy as undefined → preset defaults applied.
  // Explicit shape fixes this.
  server.tool(
    ...
    "Create a complete HeyReach campaign in one call: list + campaign + sequence (preset OR custom) + schedule + optionally start. Presets: connect_note, connect_fu, connect_note_fu. Returns campaignId + listId. Add leads separately via HeyReach UI or /lead/AddLeadsToCampaignV2.",
    {
      ...
      customMessages: z.object({
        note:               z.string().optional().describe("Connection request note (1st step). Placeholders: {FIRST_NAME}, {COMPANY}, {POSITION}, {LOCATION}, {INDUSTRY}, {ACCOUNT_NAME}"),
        noteFallback:       z.string().optional().describe("Fallback if any placeholder is empty"),
        followup:           z.string().optional().describe("Follow-up message body (used by connect_fu)"),
        followupFallback:   z.string().optional(),
        followup1:          z.string().optional().describe("First follow-up message after accept (used by connect_note_fu)"),
        followup1Fallback:  z.string().optional(),
        followup2:          z.string().optional().describe("Second follow-up if no reply"),
        followup2Fallback:  z.string().optional(),
        withdrawDays:       z.number().optional().describe("Days before withdrawing the connection request. Default 14"),
        fuDelay:            z.number().optional().describe("Days between accept and follow-up (connect_fu). Default 2"),
        fu1Delay:           z.number().optional().describe("Days between accept and 1st follow-up (connect_note_fu). Default 3"),
        fu2Delay:           z.number().optional().describe("Days between 1st and 2nd follow-up. Default 5"),
      }).optional().describe("Override preset default messages. Provide only the fields you want to override; rest fall back to preset defaults."),
