...
// Append a user turn whose content is NOT a plain string but an array of
// Anthropic content blocks (tool_result blocks, specifically).
//
// This exists because the agent loop, after executing tools, must feed the
// tool results back to Claude as a user-role message containing tool_result
// blocks (per the Anthropic Messages API spec). That content is an array, not
// a string, so it can't go through appendUserTurn (which validates string).
//
// CRITICAL: this MUST go through writeStore() so the in-memory cache stays
// consistent. An earlier implementation wrote the file directly from agent.js,
// which left this module's cache stale — getMessagesForApi() would then return
// a history MISSING the tool_result turn, causing Anthropic to reject the next
// request with "tool_use ids were found without tool_result blocks" (HTTP 400).
function appendUserTurnRaw({ content, from = "system", fromUserId = null }) {
  if (!Array.isArray(content)) {
    throw new Error("appendUserTurnRaw: content must be an array of content blocks");
  }
  if (!["anton", "pavel", "system"].includes(from)) {
    throw new Error(`from must be anton|pavel|system, got ${from}`);
  }
...
//
// SAFETY: the kept-recent window must not begin with an orphan tool_result
// user turn (a user turn whose content is tool_result blocks) — that would be
// the first message in the array and Anthropic rejects a leading tool_result
// with no preceding tool_use. If the slice would start that way, walk the
// window start backwards/forwards to a clean boundary.
function applySummarization({ newSummary, keepRecentTurns = 10 }) {
  const store = getStore();
  let recent = store.turns.slice(-keepRecentTurns);

  // Trim leading turns until the window starts with a clean user turn
  // (string content) or is empty. An assistant turn or a tool_result user
  // turn at position 0 would be an invalid conversation start.
  while (recent.length > 0) {
    const first = recent[0];
    const isToolResultTurn = first.role === "user"
      && Array.isArray(first.content)
      && first.content.some(b => b && b.type === "tool_result");
    const isAssistantFirst = first.role === "assistant";
    if (isToolResultTurn || isAssistantFirst) {
