...
function registerMcpTools(server) {
  ...
  server.tool(
    "apollo_bulk_match",
    "Enrich up to 50 Apollo people in PARALLEL via Promise.all. Each person can be specified by Apollo `id` (fastest) OR firstName+lastName+organizationName OR linkedinUrl. Returns compact filtered profiles in same order as input. Failed lookups return null at that index. Use this instead of looping apollo_match_person — it's ~10x faster wall-clock for batches and avoids context bloat from sequential responses.",
    {
    ...
    async (params) => {
      try {
        // Accept stringified payload defensively (same MCP transit issue as customMessages).
  ...
  // customMessages quirk: some MCP clients/transports stringify nested objects
  // before forwarding (observed Apr 25, 2026 — campaign 408212 received an
  // empty/undefined object on the proxy side; explicit z.object schema then
  // started rejecting calls with "Expected object, received string"). To stay
  // robust we accept z.any() here and JSON.parse defensively in the handler
  // so both shapes — real object AND JSON-stringified object — work.
  server.tool(
    ...
    async (params) => {
      try {
        // Defensive parse: some MCP transit paths deliver nested objects as
        // JSON strings. Normalize to a real object before forwarding.
        let cm = params.customMessages;
        if (typeof cm === "string") {
          try { cm = JSON.parse(cm); }
          catch { /* leave as-is, handler will fall back to preset defaults */ }
        }
  ...
  server.tool(
    ...
    async ({ path, payload }) => {
      try {
        // Same defensive parse as heyreach_create_full_campaign.
        let pl = payload;
        if (typeof pl === "string") {
