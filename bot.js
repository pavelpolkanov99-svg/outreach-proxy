...
const SALES_CHAT_ID = process.env.SALES_CHAT_ID
  ? parseInt(process.env.SALES_CHAT_ID.trim(), 10)
  : -1003888902313;
...
// ─────────────────────────────────────────────────────────────────────────────
// In-memory task buffer (Business messages from Anton)
// ─────────────────────────────────────────────────────────────────────────────

// pendingAntonMessages: { id, text, timestamp }[]
// Survives until next morning cron — then cleared after digest sent
const pendingAntonMessages = [];

function addAntonMessage(text) {
  const id = `task_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
  pendingAntonMessages.push({ id, text: text.trim(), timestamp: new Date().toISOString() });
  console.log(`[task-detector] buffered message from Anton: "${text.slice(0, 80)}..."`);
  return id;
}

function clearAntonMessages() {
  pendingAntonMessages.splice(0, pendingAntonMessages.length);
}

// ─────────────────────────────────────────────────────────────────────────────
// Task detection via Claude
// ─────────────────────────────────────────────────────────────────────────────

async function detectTasksInMessages(messages) {
  if (!messages.length) return [];
  const combined = messages.map((m, i) => `[${i + 1}] ${m.text}`).join("\n");
  try {
    const resp = await axios.post(`${PROXY}/anthropic/detect-tasks`, { messages: combined }, { timeout: 30_000 });
    return resp.data?.tasks || [];
  } catch (_) {
    // Fallback: heuristic detection
    const taskKeywords = /сделай|проверь|напиши|отправь|позвони|договорись|обнови|добавь|скинь|свяжись|подготовь|поставь|найди|уточни|перешли|дай знать|нужно|надо|можешь|можешь|помоги/i;
    return messages
      .filter(m => taskKeywords.test(m.text))
      .map(m => ({ id: m.id, text: m.text, confidence: "heuristic" }));
  }
}
...
bot.on("business_message", async (ctx) => {
  try {
    ...
    // Only messages FROM Anton
    if (senderId !== ANTON_TG_ID) return;
    if (!text.trim()) return;

    addAntonMessage(text);
    console.log(`[business_message] Anton → buffered (${pendingAntonMessages.length} total)`);
  } catch (err) {
    console.error("[business_message] handler error:", err.message);
  }
});

// ─────────────────────────────────────────────────────────────────────────────
// Morning task digest (08:00 Prague)
// ─────────────────────────────────────────────────────────────────────────────

async function sendTaskDigest() {
  console.log("[cron] task-digest start — buffered messages:", pendingAntonMessages.length);

  if (!pendingAntonMessages.length) {
    console.log("[cron] task-digest: no messages from Anton, skipping");
    return;
  }

  const snapshot = [...pendingAntonMessages];
  clearAntonMessages();

  let detectedTasks;
  try {
    detectedTasks = await detectTasksInMessages(snapshot);
  } catch (err) {
    console.error("[cron] task-digest: detection failed:", err.message);
    detectedTasks = snapshot.map(m => ({ id: m.id, text: m.text, confidence: "fallback" }));
  }

  if (!detectedTasks.length) {
    console.log("[cron] task-digest: no tasks detected");
    await bot.api.sendMessage(PAVEL_TG_ID,
      `📋 <b>Задачи от Антона</b> · утренний дайджест\n\n<i>Сообщений за ночь: ${snapshot.length}\nЗадач не обнаружено.</i>`,
      { parse_mode: "HTML" }
    );
    return;
  }

  const headerDate = buildRussianHeaderDate();
  await bot.api.sendMessage(PAVEL_TG_ID,
    `📋 <b>Задачи от Антона</b> · ${headerDate}\n\n<i>Обнаружено ${detectedTasks.length} из ${snapshot.length} сообщений.\nВыбери что постить в sales чатик:</i>`,
    { parse_mode: "HTML" }
  );

  for (const task of detectedTasks) {
    const taskId = task.id || `task_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
    const text = task.text.length > 3000 ? task.text.slice(0, 2997) + "..." : task.text;
    const conf = task.confidence === "heuristic" ? " · <i>эвристика</i>" : "";
    await bot.api.sendMessage(PAVEL_TG_ID,
      `📌 <b>Задача</b>${conf}\n\n${esc(text)}`,
      {
        parse_mode: "HTML",
        reply_markup: {
          inline_keyboard: [[
            { text: "✅ Постить в sales", callback_data: `post_task:${encodeTaskPayload(taskId, text)}` },
            { text: "❌ Пропустить",      callback_data: `skip_task:${taskId}` },
          ]]
        }
      }
    );
...
function encodeTaskPayload(id, text) {
  // Store task text in memory keyed by id to avoid callback_data length limit
  taskPayloadStore[id] = text;
  return id;
}

// In-memory store for task texts referenced by callback buttons
const taskPayloadStore = {};

// ─────────────────────────────────────────────────────────────────────────────
// Callback: post_task / skip_task
// ─────────────────────────────────────────────────────────────────────────────
...
async function handleTaskCallback(ctx) {
  ...
  if (data.startsWith("post_task:")) {
    const taskId = data.slice("post_task:".length);
    const taskText = taskPayloadStore[taskId];
    delete taskPayloadStore[taskId];

    if (!taskText) {
      await ctx.answerCallbackQuery({ text: "Задача уже обработана или устарела" });
      try { await ctx.editMessageReplyMarkup({ reply_markup: { inline_keyboard: [] } }); } catch (_) {}
      return;
    }

    await ctx.answerCallbackQuery({ text: "✅ Постим в sales..." });

    try {
      await bot.api.sendMessage(SALES_CHAT_ID,
        `📌 ${taskText}`,
        { parse_mode: "HTML" }
      );
      try {
        await ctx.editMessageText(
          ctx.message?.text?.replace(/^📌/, "✅") || `✅ Запостил в sales:\n\n${esc(taskText)}`,
          { parse_mode: "HTML", reply_markup: { inline_keyboard: [] } }
        );
      } catch (_) {}
      console.log(`[task-callback] posted to sales chat: "${taskText.slice(0, 80)}"`);
    } catch (err) {
      console.error("[task-callback] failed to post to sales:", err.message);
      await bot.api.sendMessage(PAVEL_TG_ID,
        `❌ Не удалось запостить в sales чатик: ${esc(err.message)}`,
        { parse_mode: "HTML" }
      );
    }
    return;
  ...
  if (data.startsWith("skip_task:")) {
    const taskId = data.slice("skip_task:".length);
    delete taskPayloadStore[taskId];
    await ctx.answerCallbackQuery({ text: "Пропущено" });
    try {
      await ctx.editMessageText(
        (ctx.message?.text || "").replace(/^📌/, "⏭") || "⏭ Пропущено",
        { parse_mode: "HTML", reply_markup: { inline_keyboard: [] } }
      );
    } catch (_) {}
    return;
...
async function handleApprovalCallback(ctx) {
  ...
  // Task callbacks take priority
  if (data.startsWith("post_task:") || data.startsWith("skip_task:")) {
    return handleTaskCallback(ctx);
  }
...
bot.command("test_tasks", ctx => guard(ctx, async () => {
  await ctx.reply("⏳ Запускаю тест task digest прямо сейчас...");
  // Add a couple of test messages if buffer is empty
  if (!pendingAntonMessages.length) {
    addAntonMessage("Паш, нужно сделать апдейт по FreeMarket — отправь им follow-up сегодня");
    addAntonMessage("Кстати, надо созвониться с командой по Nairobi итогам на этой неделе");
    addAntonMessage("Привет! Как дела?");
  }
  await sendTaskDigest();
  await ctx.reply("✅ Тест task digest запущен — проверь личку.");
}));
...
bot.on("callback_query:data", async (ctx) => {
  ...
  const data = ctx.callbackQuery?.data || "";
  // Task callbacks don't require CONVERSATIONAL_MODE
  if (data.startsWith("post_task:") || data.startsWith("skip_task:")) {
    try { await handleTaskCallback(ctx); } catch (err) { console.error("[bot] task callback error:", err.message); }
    return;
  }
...
// 08:00 CET — Anton task digest → Pavel личка only
cron.schedule("0 8 * * *", sendTaskDigest, { timezone: "Europe/Prague" });
console.log("[cron] task digest registered (08:00 Europe/Prague → Pavel личка)");
...
console.log(`[bot] Sales chat ID: ${SALES_CHAT_ID}`);
