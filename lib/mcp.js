...
//   Parallel:  6
//   Notion:    10 (query, update_company, update_tags, append_note,
//                  check_duplicates, upsert_person, page_blocks,
//                  discovery_card_fill, update_page_props,
//                  [update_company w/ comm_channel])
//   Apollo:    4  (search_person, match_person, bulk_match, people_search)
//   HeyReach:  4
...
function registerMcpTools(server) {
  ...
  server.tool("notion_page_blocks",
    "Read all blocks of a Notion page recursively. Returns compact block tree with text, type, checked state. Use for reading Discovery Card or any page content.",
    { pageId: z.string(), depth: z.number().optional() },
    async ({ pageId, depth }) => {
      try {
        const qs = depth ? `?depth=${depth}` : "";
        return asText(await selfGet(`/notion/page-blocks/${pageId}${qs}`, 30000));
      } catch(e) { return asError(e); }
    }
  );

  server.tool("notion_discovery_card_fill",
    "Score a Discovery Card page by fill percentage across all 7 sections (Partner Profile, Business Profile, Client Mix, Stablecoins, Volumes, Pain Points, Tech). Returns { fillPct, sections, totalFilled, totalFields } — use for ranking cards and building ping list.",
    { pageId: z.string() },
    async ({ pageId }) => {
      try { return asText(await selfGet(`/notion/discovery-card-fill/${pageId}`, 30000)); } catch(e) { return asError(e); }
    }
  );

  server.tool("notion_update_page_props",
    "Update arbitrary rich_text properties on ANY Notion page by direct pageId — use for custom databases not in the CRM. Body: pageId + props object where each value is a string (written as rich_text). Example: { pageId: 'xxx', props: { 'Insight': 'text...', 'Corridor': 'USD/EUR...' } }",
    {
      pageId: z.string().describe("Notion page ID (UUID with dashes)"),
      props: z.record(z.string()).describe("Object mapping field names to string values"),
    },
    async ({ pageId, props }) => {
      try { return asText(await selfPost("/notion/update-page-props", { pageId, props })); } catch(e) { return asError(e); }
    }
  );
  ...
  server.tool("apollo_people_search",
    "Find people at a company filtered by title (e.g. 'General Partner', 'Managing Partner', 'Partner', 'Founder'). Resolves the organization first (by domain via org enrich, or by name via organizations/search — best text match), then searches Apollo scoped to that organization_id. Use this to build fund/company contact lists (e.g. '3-5 general partners at Pantera Capital') instead of apollo_search_person, which requires an already-known individual name. Returns up to 25 people with title, seniority, email, LinkedIn.",
    {
      organizationName: z.string().optional().describe("Company/fund name, e.g. 'Pantera Capital'. Used if domain not given."),
      domain: z.string().optional().describe("Company domain, e.g. 'panteracapital.com'. More reliable than name when known."),
      titles: z.array(z.string()).optional().describe("Job titles to filter by. Defaults to ['General Partner','Managing Partner','Partner','Founder','Co-Founder']."),
      perPage: z.number().optional().describe("Max results, default 10, hard cap 25."),
    },
    async (params) => {
      try { return asText(await selfPost("/apollo/people-search", { apolloKey: DEFAULT_APOLLO_KEY, ...params })); } catch(e) { return asError(e); }
    }
  );
