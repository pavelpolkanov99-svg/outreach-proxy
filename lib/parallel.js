...
//
// v2.2 changelog (May 8 2026, Anton's discipline patches):
// - Mandatory Hard Kill STOP — no overrides allowed by skill consumer
// - Mandatory FACT/INFERENCE/UNKNOWN labels per axis
// - UNKNOWN axes capped at 3/10 (enforced in routes/parallel.js compact builder)
// - Brand recognition trap explicit warning
// - Axis 8 max=6 for direct primary-product overlap
// - SE signal kept (applied as band-locked +0.5 in routes/parallel.js)
// - Tier mapping moved to routes/parallel.js: P1 ≥7.5 / P2 5.0-7.49 / P3 3.0-4.99 / Skip <3.0
// - MH tier removed (was inflated)
//
function buildResearchQuery(company, domain) {
  return [
    ...
    `For EACH axis (1, 2, 3, 4, 5, 6, 7, 8, 10), set axisN_meta to one of:`,
    `  - "FACT": directly stated in cited sources (company website, regulator register, reputable press)`,
    `  - "INFERENCE": derived from indirect evidence in sources (e.g. partner list → likely stablecoin alignment)`,
    `  - "UNKNOWN": no evidence found in sources for this axis`,
    `If evidence is UNKNOWN, set the score to your best guess BUT mark axisN_meta="UNKNOWN".`,
...
// v2.2 schema: 18 properties + 9 axis_meta = 27 total, 24 required
  ...
  output_schema: {
    ...
    json_schema: {
      ...
      properties: {
        ...
        // ── Axes meta (v2.2 NEW) — FACT/INFERENCE/UNKNOWN per axis ────────
        axis1_meta:  { type: "string", enum: ["FACT", "INFERENCE", "UNKNOWN"] },
        axis2_meta:  { type: "string", enum: ["FACT", "INFERENCE", "UNKNOWN"] },
        axis3_meta:  { type: "string", enum: ["FACT", "INFERENCE", "UNKNOWN"] },
        axis4_meta:  { type: "string", enum: ["FACT", "INFERENCE", "UNKNOWN"] },
        axis5_meta:  { type: "string", enum: ["FACT", "INFERENCE", "UNKNOWN"] },
        axis6_meta:  { type: "string", enum: ["FACT", "INFERENCE", "UNKNOWN"] },
        axis7_meta:  { type: "string", enum: ["FACT", "INFERENCE", "UNKNOWN"] },
        axis8_meta:  { type: "string", enum: ["FACT", "INFERENCE", "UNKNOWN"] },
        axis10_meta: { type: "string", enum: ["FACT", "INFERENCE", "UNKNOWN"] },
        // ── Meta fields ───────────────────────────────────────────────────
        top_sources: {
          type: "array",
      ...
      required: [
        ...
        "axis1_meta", "axis2_meta", "axis3_meta", "axis4_meta",
        "axis5_meta", "axis6_meta", "axis7_meta", "axis8_meta",
