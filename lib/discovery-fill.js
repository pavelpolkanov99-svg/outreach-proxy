/**
 * Discovery Card fill scoring — structural approach v2
 *
 * "Filled" = partner actually typed/checked something NEW.
 * "Empty"  = only our pre-fill template content.
 *
 * Key insight: paragraphs in "Label: Value" format where the Label
 * is one of our known field labels are boilerplate — we put them there.
 * Only standalone values WITHOUT a label prefix count as partner input.
 *
 * Rules:
 ...
 *   standalone short word like "USDT","USDC"    → empty (our pre-fill in column)
 *   paragraph with real partner content         → filled
 *   table_row (non-header) with real data       → filled
 */

"use strict";

// ── Known field label prefixes (we pre-fill these as "Label: ___") ─────────────
// Any paragraph starting with one of these is our template, not partner input.
...
// ── Known boilerplate prose blocks (section intros, instructions, etc.) ────────
const BOILERPLATE_PATTERNS = [
  ...
  /^where you want to move money/i,
  // Stablecoin/chain names as standalone paragraphs (our pre-fill column headers)
  /^(usdt|usdc|eurc|eure|usde|dai|pyusd|busd|tusd|usdp)$/i,
  // Short standalone chain names (our pre-fill)
...
function isBoilerplate(text) {
  ...
  // Known field label prefix (we put these in the card)
  ...
  // Pure label ending with colon (short)
  ...
  function walk(blist) {
    for (const b of blist) {
      ...
      if (type === "table_row") {
        ...
        // Skip header rows
        const isHeader = nonEmpty.every(c => /^[A-Z]/.test(c.trim()) && c.trim().length < 60 && !/\d/.test(c));
