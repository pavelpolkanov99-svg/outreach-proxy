...
//   Notion:    6  (query, update_company, update_tags, append_note, check_duplicates, update_company_with_tags)
...
function registerMcpTools(server) {
  ...
  server.tool(
    ...
    "Update fields on a Notion Companies CRM entry by company name. Covers all BD scoring fields: bd_score, priority, corridors, stage, tags (Tags multi-select — use tags param for Outreach/etc). PREFERRED over calling notion_update_tags separately — handles both fields in one call.",
    {
      ...
      tags: z.array(z.string()).optional().describe("Tags to ADD (additive, does not overwrite existing). e.g. ['Outreach', 'Money2020']"),
      description: z.string().optional(),
      website: z.string().optional(),
      location: z.string().optional(),
      source: z.string().optional().describe("Notion select field — no commas. Format: 'BD Scoring MonYYYY'"),
  ...
  server.tool(
    "notion_update_tags",
    "Add or replace Tags (multi-select) on a Notion Companies or People entry. Use mode='add' (default) to append without overwriting existing tags. Use mode='replace' to set tags exactly. For BD scoring writeback, prefer notion_update_company with tags param instead.",
    {
  ...
  server.tool(
    ...
    "Bulk CRM dedup check — given an array of company names, returns which ones already exist in the Notion Companies DB and their current Stage. Use before creating HeyReach campaigns to avoid re-contacting companies already in the pipeline. Single call replaces N separate notion_update_company probes.",
    {
  ...
  server.tool(
    "apollo_match_person",
    "Apollo people enrichment by Apollo ID OR name+company OR LinkedIn URL. Returns COMPACT filtered profile (LinkedIn URL, email, title, company, funding etc — ~1KB instead of raw 5KB). Pass `id` for fastest most-reliable lookup when you have it from a prior search.",
    {
      id: z.string().optional().describe("Apollo person id (from a prior people search). Most reliable."),
  ...
  server.tool(
    "apollo_bulk_match",
    "Enrich up to 50 Apollo people in PARALLEL via Promise.all. Each person can be specified by Apollo `id` (fastest) OR firstName+lastName+organizationName OR linkedinUrl. Returns compact filtered profiles in same order as input. Failed lookups return null at that index.",
    {
      people: z.array(z.object({
        ...
        domain:           z.string().optional(),
        linkedinUrl:      z.string().optional(),
      })).min(1).max(50).describe("List of person specs. Each spec needs at least one identifier."),
    },
    async (params) => {
      try {
        let people = params.people;
        if (typeof people === "string") {
          try { people = JSON.parse(people); } catch { /* ignore */ }
        }
        const data = await selfPost("/apollo/bulk-match", {
          apolloKey: DEFAULT_APOLLO_KEY,
          people,
        }, 60000);
        return asText(data);
      } catch (e) { return asError(e); }
  ...
  server.tool(
    ...
    "Create a complete HeyReach campaign in one call: list + campaign + sequence (preset OR custom) + schedule + optionally start. Presets: connect_note, connect_fu, connect_note_fu. customMessages keys: note, noteFallback, followup1, followup1Fallback, followup2, followup2Fallback, followup, followupFallback, withdrawDays (default 14), fuDelay, fu1Delay, fu2Delay. Returns campaignId + listId + customMessagesApplied (echo of which keys were used).",
    {
      ...
      customMessages: z.any().optional().describe("Override preset default messages. Pass an object with keys like {note, followup1, followup2}. Stringified JSON is also accepted and auto-parsed."),
  ...
  server.tool(
    ...
    async ({ path, payload }) => {
      try {
        let pl = payload;
        if (typeof pl === "string") {
          try { pl = JSON.parse(pl); } catch { /* keep as-is */ }
        }
        const data = await selfPost("/heyreach/proxy", {
          hrKey: DEFAULT_HEYREACH_KEY, path, payload: pl,
        });
        return asText(data);
      } catch (e) { return asError(e); }
    }
    ...
    async ({ path, ref }) => {
      try {
        ...
        return asText({
          ...
          encoding: data.encoding, content: decoded, html_url: data.html_url,
        });
      } catch (e) { return asError(e); }
    }
  );
    ...
    async ({ path, content, message, branch = "main" }) => {
      try {
        ...
        return asText({
          ...
          commit_sha: data.commit?.sha, commit_url: data.commit?.html_url,
  ...
  server.tool(
    ...
    `Create a new branch in ${GITHUB_OWNER}/${GITHUB_REPO} from an existing source branch (default: main).`,
    {
