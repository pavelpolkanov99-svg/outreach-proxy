...
//
// MVP scope: 20 hand-picked tools across Notion, Apollo, Parallel, Beeper,
...
const TOOLS = [
  // ─── Notion ────────────────────────────────────────────────────────────────
  {
    ...
    handler: async (input) => {
      ...
      // Trim massive Notion responses — keep only essentials per result.
  ...
  // ─── Apollo ────────────────────────────────────────────────────────────────
  {
    ...
    definition: {
      ...
      description: "Search Apollo.io for a person by name and company. Returns matching profiles with LinkedIn, title, email (if available).",
      input_schema: {
        type: "object",
        properties: {
          firstName: { type: "string" },
    ...
    handler: async (input) => {
      const r = await callProxy("POST", "/apollo/search-person", { data: input });
...

  {
    ...
    definition: {
      ...
      description: "Enrich data for one person via Apollo. Provide Apollo ID, OR name+company, OR LinkedIn URL. Returns title, email, phone, LinkedIn profile info.",
      input_schema: {
        type: "object",
        properties: {
    ...
    handler: async (input) => {
      const r = await callProxy("POST", "/apollo/match-person", { data: input });
      return r.ok ? truncateForClaude(r.data) : { error: r.error };
    },
  },
  ...
  // ─── Parallel.ai ────────────────────────────────────────────────────────────
  {
    ...
    definition: {
      ...
      input_schema: {
        ...
        properties: {
          company:   { type: "string" },
          website:   { type: "string", description: "Company website (recommended for accuracy)" },
          context:   { type: "string", description: "Optional context (e.g. 'met at Money20/20')" },
...

  {
    ...
    handler: async (input) => {
      const r = await callProxy("GET", `/parallel/get-result`, { params: input, timeout: 15_000 });
...

  {
    ...
    approvalSummary: (input) => {
      ...
      lines.push(`• Company: <b>${input.company}</b>`);
      if (input.website) lines.push(`• Website: ${input.website}`);
      lines.push(`• Cost: ~$0.025 (${input.processor || "core"})`);
      return lines.join("\n");
    },
    handler: async (input) => {
  ...
  // ─── Beeper ─────────────────────────────────────────────────────────────────
  {
    ...
    definition: {
      ...
      input_schema: {
        type: "object",
        properties: {
          days:  { type: "integer", description: "Look-back window (default 1, max 7)" },
          limit: { type: "integer", description: "Max chats (default 25, max 50)" },
        },
        required: [],
      },
    },
...

  {
    ...
    definition: {
      ...
      description: "Find Beeper chat(s) by fuzzy name match. Returns chat metadata + recent messages. Use when user says 'show me chat with Bitso' or 'what did John say last week'.",
      input_schema: {
        type: "object",
        properties: {
...

  {
    ...
    definition: {
      ...
      description: "Send a text message to a Beeper chat. CRITICAL: This sends a real message to a real person. Always double-check chat name and message content before approving. Pass chatID from beeper_get_conversation or beeper_digest.",
      input_schema: {
        type: "object",
        properties: {
          chatID:  { type: "string", description: "Beeper chat ID" },
    ...
    approvalSummary: (input) => {
      ...
      lines.push(`• Chat ID: <code>${input.chatID.slice(0, 40)}...</code>`);
      lines.push(`• Текст:\n"${input.text}"`);
      return lines.join("\n");
    },
    handler: async (input) => {
      const r = await callProxy("POST", "/beeper/send-message", { data: input, timeout: 15_000 });
      return r.ok ? r.data : { error: r.error };
    },
  },
  ...
  // ─── Google Calendar ───────────────────────────────────────────────────────
  {
    ...
    definition: {
      ...
      description: "List Anton's calendar events for a time range. Returns events with summary, time, attendees (external only), Google Meet link, and CRM enrichment if attendee email matches a Notion company.",
      input_schema: {
        type: "object",
        properties: {
          when:  { type: "string", description: "today | tomorrow | week | YYYY-MM-DD" },
    ...
    handler: async (input) => {
      // Map convenience strings to endpoint
      ...
      // For other ranges, use /calendar/range with computed boundaries
      // (simplification — extend later if Anton actually asks for "tomorrow" etc)
...
// Returns Anthropic-format tool definitions array (input to API call)
function getToolDefinitions() {
  return TOOLS.map(t => t.definition);
}

// Returns tool count broken down by read/write
...
// Returns the tool object for a given name (with handler, isWrite, approvalSummary)
