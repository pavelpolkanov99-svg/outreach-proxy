...
## CRITICAL — exact property names (Notion filters fail on wrong names)

When building a notion_query filter you MUST use the EXACT property name for
that database. The two databases do NOT share the same title property name —
this is the single most common mistake, so check this list every time:
...
So: to find a company by name, filter on "Company name". To find a person by
name, filter on "Name". Mixing these up returns
...
- If a notion_query fails with "Could not find property", do NOT blindly retry —
  re-read the property list above and fix the property name, then retry once.
